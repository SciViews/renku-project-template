...
#  creds_file <- file.path(renku_dir, ".git", "credentials")
#  creds_save_file <- file.path(config_dir, "credentials")
#  if (file.exists(creds_file))
#    file.copy(creds_file, creds_save_file, overwrite = TRUE)
#
...
setHook("rstudio.sessionInit", function(newSession) {
  if (newSession) {
    if (file.exists(".config/renku_restore.R"))
